...
char PPM::setUserChoice(){
   std::cout << "How would you like to convert"
    " your PPM image: " << std::endl <<
    "[c]opy image" << std::endl <<
    "[r]ed output" << std::endl <<
    "[g]reen output" << std::endl <<
    "[b]lue output" << std::endl <<
    "[l]inear output" << std::endl <<
    "[n]one of these apply" << std::endl <<
    std::endl << "Please enter your choice: ";
    std::cin >> mUserChoice;
...
PPM PPM::operator=(const PPM& rhs){
        this->mMagicNumber = rhs.mMagicNumber;
        this->mWidth = rhs.mWidth;
        this->mHeight = rhs.mHeight;
        this->mMaxColorValue = rhs.mMaxColorValue;
        this->mUserChoice = rhs.mUserChoice;
        this->binaryRepresentation = rhs.binaryRepresentation;
        this->binRep = rhs.binRep;

        return *this;
}

PPM PPM::outputCopy(const PPM& rhs){
    *this = rhs;
    return *this;
...
PPM PPM::outputRed(){
    ...
    for(row = 0; row <= mHeight; row++){
        for(column = 0;
            column <= mWidth;
            column++){
            int currentRed = getChannel(row, column, 0);
            for(channel = 0;
                channel < 3;
                channel++){
                setChannel(row,
...
PPM PPM::outputGreen(){
    ...
    for(row = 0; row <= mHeight; row++){
        for(column = 0;
            column <= mWidth;
            column++){
            int currentGreen = getChannel(row, column, 1);
            for(channel = 0;
                channel < 3;
                channel++){
                setChannel(row,
...
PPM PPM::outputBlue(){
    ...
    for(row = 0; row <= mHeight; row++){
        for(column = 0;
            column <= mWidth;
            column++){
            int currentBlue = getChannel(row, column, 2);
            for(channel = 0;
                channel < 3;
                channel++){
                setChannel(row,
    ...
    for(row = 0; row <= mHeight; row++){
        for(column = 0;
            ...
            for(channel = 0;
                ...
                channel++){
                    int red =
                    getChannel(row, column, channel);
                    int green =
                    getChannel(row, column, channel);
                    int blue =
                    getChannel(row, column, channel);
                    int currentValue =
                    (0.2126*red) + (0.7152*green)
                    + (0.0722*blue);
                    setChannel(row,
                               column,
                               channel,
